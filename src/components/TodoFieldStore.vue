...
export default {
  ...
  computed: {
    ...
    managers() {
      return Object.values(
        this.$store.getters.getValue({ key: "manager", value: "byId" })
      );
    },
  },
  mounted() {
  ...
  methods: {
    onSubmit: function () {
      this.$store.commit("createOrUpdate", { key: "todo", value: this.myTodo });
      this.clearTodo();
      this.$store.commit("setCurrentTodo", {});
    },
    initData() {
    ...
    clearTodo() {
      this.myTodo = { task: "", hours: 1, manager: 1 };
      this.isUpdate = false;
    },
  },
  watch: {
